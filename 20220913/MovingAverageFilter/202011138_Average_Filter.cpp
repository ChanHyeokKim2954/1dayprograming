...
double AverageFilter(double average_old, double data, int size)
{
	double average = 0.0;
	double alpha = 0.0;
...
int main(void)
{
	...
	while (1)
	{
		...
		array[count] = data;
		calculation = AverageFilter(average_old, data, size);
		average_old = PreAverage(size);
