...
nav_msgs::Odometry delta,data,old_data,prev_data;
...
void odomCallback(const nav_msgs::Odometry& msg)
{
	...
	float KI = 1.0;
	float set_data = 3.0;
	float error, delta_error = 0.0;
	float proportional = 0.0, integral = 0.0, differential = 0.0;
	float input_data = 0.0, output_data = 0.0;
	...
	input_data = sqrt(pow(data.pose.pose.position.x,2)+pow(data.pose.pose.position.y,2));
	//ROS_INFO("input_data = [%lf]",input_data);
	error = set_data - input_data;
	proportional = KP * error;
	integral += KI * error * 0.02;
	differential = KD * delta_error / 0.02;
	delta_error = error;
	output_data = proportional + integral + differential;
	ROS_INFO("output_data = [%lf]",output_data);
	if(output_data <= 0.0)
	{
