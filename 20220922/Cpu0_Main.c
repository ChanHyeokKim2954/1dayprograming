...
int adc_value = 0 ;
float adc_filter_old = 0.0, adc_filter = 0.0, angle = 0.0;
float pmw_duty = 0.0;
...
int core0_main(void)
{
    ...
    while(1)
    {
        ...
        adc_filter = ALPHA * adc_filter + (1-ALPHA) * (float)adc_value;
        angle = map(adc_filter, 0.0, 4095.0, 0.0,180.0);
        Set_GptPWM_Duty(angle);
        //wait(ticksFor100ms);
    }
    return (1);
}
