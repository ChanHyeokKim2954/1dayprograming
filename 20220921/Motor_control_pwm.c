...
#define IN1  &MODULE_P00,9
#define IN2  &MODULE_P00,10
...
void init_PWM_signal_generation(void)
{
    ...
    IfxPort_setPinModeOutput(IN1, IfxPort_OutputMode_pushPull, IfxPort_OutputIdx_general);
    IfxPort_setPinLow(IN1);
    IfxPort_setPinModeOutput(IN2, IfxPort_OutputMode_pushPull, IfxPort_OutputIdx_general);
    IfxPort_setPinLow(IN2);
}

/* Generation of a simple PWM signal by toggling a port pin (frequency can be changed during runtime) */
int generate_PWM_signal(int pwm_data)
{
    ...
    g_calculatedPwmDutyCycle =( (float)pwm_data * 100 )/ 255;
   ...
   pwm =( g_calculatedPwmDutyCycle * 255 )/ 100;
   return pwm;
}

int PWM_map(int data, int in_min, int in_max, int out_min, int out_max)
{

    map = (data - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
    return map;
}

void DCmotor(int pwm_data)
{
    if(pwm_data < 0)
    {
        IfxPort_setPinHigh(IN1);
        IfxPort_setPinLow(IN2);
    }
    else if(pwm_data > 0)
    {
        IfxPort_setPinLow(IN1);
        IfxPort_setPinHigh(IN2);
    }
    else
    {
        IfxPort_setPinLow(IN1);
