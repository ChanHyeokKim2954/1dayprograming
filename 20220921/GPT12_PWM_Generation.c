...
{
    map_value = (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
    return map_value;
}
extern float pmw_duty;
void Set_GptPWM_Duty(float angle)
{
    pmw_duty = map(angle, 0.0, 180.0, 2.5, 12.0);
    double dutyUpTime   = (fTimer * (pmw_duty / 100.0f)) / PWM_FREQUENCY;
    double dutyDownTime   = (fTimer *(1 - (pmw_duty / 100.0f))) / PWM_FREQUENCY;
    IfxGpt12_T2_setTimerValue(&MODULE_GPT120, dutyDownTime);
    IfxGpt12_T4_setTimerValue(&MODULE_GPT120, dutyUpTime);
}
